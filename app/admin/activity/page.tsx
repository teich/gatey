...
export default function ActivityPage() {
  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 p-4 md:p-6 lg:p-8">
    ...
    <div className="admin-table-shell"><Table><TableHeader><TableRow><TableHead sortKey="when">When</TableHead><TableHead sortKey="who">Who</TableHead><TableHead sortKey="household">Household</TableHead><TableHead sortKey="action">Action</TableHead><TableHead sortKey="result">Result</TableHead><TableHead sortKey="record">Gatey&apos;s record</TableHead></TableRow></TableHeader><TableBody>
      {events.map((event) => <TableRow key={event.id} sortValues={{ when: event.occurredAt, who: event.actorName, household: event.householdName, action: actionLabel(event), result: event.outcome, record: detailLabel(event) }}><TableCell className="whitespace-nowrap text-sm text-muted-foreground">{formatDate(event.occurredAt)}</TableCell><TableCell><strong>{event.actorName}</strong></TableCell><TableCell>{event.householdName || "No household"}</TableCell><TableCell>{actionLabel(event)}</TableCell><TableCell><span className={event.outcome === "succeeded" ? "managed-badge" : "existing-badge"}>{event.outcome === "succeeded" ? "Completed" : "Failed"}</span></TableCell><TableCell className="text-sm text-muted-foreground">{detailLabel(event)}</TableCell></TableRow>)}
