    ...
    {open ? <div className="dialog-backdrop" role="presentation">
      <section className="dialog admin-assignment-dialog" role="dialog" aria-modal="true" aria-labelledby={`assign-${personId}`}>
        ...
        {welcome ? <>
          ...
          <label>Gatey account<select value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="new">Create a Gatey account</option>{eligibleAccounts.map((account) => <option key={account.id} value={account.id}>{account.name} · {account.email || account.username || "No email"}{account.householdName ? ` · ${account.householdName}` : ""}</option>)}</select></label>
          {accountId === "new" ? <div className="admin-assignment-fields"><label>Name<input name="name" defaultValue={personName} required /></label><label>Email <span className="font-normal text-muted-foreground">Optional</span><input name="email" type="email" placeholder="person@example.com" /><span className="font-normal text-muted-foreground">Only needed if this person will sign in by email.</span></label><label>Username<input name="username" required minLength={3} autoCapitalize="none" placeholder="username" /></label></div> : null}
